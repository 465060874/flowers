...
object OpenCVUtils {
  ...
  def watershedSegmentation(image: Mat): Mat = {
    val hsvImage = new Mat()
    cvtColor(image, hsvImage, COLOR_BGR2HSV)
    val binary = new Mat()
    val grayscale = new Mat()
    cvtColor(image, grayscale, COLOR_BGR2GRAY)    
    threshold(grayscale, binary, histAVG(hsvImage)*1.1, 179, THRESH_BINARY)
   // show(binary, "Watersheds")
   ...
   // show(bg, "Dilated")
    ...
    val result = new Mat()
    val mask = new Mat( image.size(),image.`type`())
    
    val segI = segmentation.createIndexer().asInstanceOf[UByteBufferIndexer]
    
    //System.out.println(segI.get(2,2));
    threshold(segmentation, mask, 129, 255, THRESH_BINARY)
    //mask.convertTo(mask,image.`type`())
    //this.show(mask, "mask")
  ...
  def histAVG(image : Mat): Double = {
    val hc = new ColorHistogram()
    val histogram = hc.getHueHistogram(image)
    var sum = 0.0
    val numberOfBins = histogram.size().height()
    val histI = histogram.createIndexer().asInstanceOf[FloatBufferIndexer]
    for (bin <- 0 until numberOfBins) {
      val add = histI.get(bin) * bin
      sum = sum + add
    }

    val res =  sum/(image.size().height()*image.size().width())
    res
  }
}